...
function setLocalPath(req, res, next) {
	...
	req.localPath = path.normalize(config.userFilesFolder+"/"+req.relLocalPath);	
	next();
}
...
router.post('/:userFolder/\*', bodyParser.raw(), setLocalPath, privateFileAuth,
	async (req, res, next) => {
		try {
			return res.status(201).send(await fsutils.writeFile(req.localPath, req.body, req.params.userFolder));
		} catch(err) {
			return next(err);
		}
	}
...
router.delete('/:userFolder/\*', express.json(), setLocalPath, privateFileAuth,
	async (req, res, next) => {
		try {
			// If it's a DIRECTORY
			if(await fsutils.isDirectory(req.localPath)) {
				return res.status(204).json(await fsutils.rmdir(req.localPath, req.params.userFolder));
			}
			
			// If it's a FILE
			else if(await fsutils.exists(req.localPath)) {
				return res.status(204).send(await fsutils.rm(req.localPath, req.params.userFolder));
			}
	...
	async (req, res, next) => {
		try {
			...
			if(req.body.name) {
				...
				await fsutils.mkdir(req.localPath, req.body.name, req.params.userFolder)
			...
			if(req.body.public !== undefined) {
				await fsutils.setPublic(pathToSet, Boolean(req.body.public), req.params.userFolder);
